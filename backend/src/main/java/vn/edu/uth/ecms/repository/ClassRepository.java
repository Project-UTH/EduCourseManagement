...
public interface ClassRepository extends JpaRepository<ClassEntity, Long> {
     ...
     */
    @Query("SELECT c FROM ClassEntity c WHERE c.semester.semesterId = :semesterId AND c.status = :status")
     ...
     * - Same day of week
     * - Same time slot
     * - Different class (exclude current class)
     */
     ...
     */
    @Query("SELECT COALESCE(SUM(c.enrolledCount), 0) FROM ClassEntity c WHERE c.semester.semesterId = :semesterId")
     ...
     */
    @Query("SELECT c FROM ClassEntity c WHERE c.semester.semesterId = :semesterId ORDER BY c.classCode ASC")
